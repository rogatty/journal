...
const AwsSigner = function(config) {
  ...
  this.payloadSerializer =
    this.config.payloadSerializer || this.config.payloadSerializerFactory();
 ...
 * @param {Date=} signDate Optional signature date to use. Current date-time is used if not
 *   specified.
...
function prepare(self, ws) {
  ...
  if (ws.request.headers["content-type"]) {
    ws.request.headers["content-type"] = ws.request.headers[
      "content-type"
    ].split(";")[0];
  }
...
function buildCanonicalRequest(self, ws) {
  ...
  ws.canonicalRequest =
    ...
    Object.keys(ws.uri.queryParams)
      .sort()
      .map(function(key) {
        return uriEncode(key) + "=" + uriEncode(ws.uri.queryParams[key]);
      })
      .join("&") +
    "\n" +
    // Canonical Headers:
...
function calculateSignature(self, ws) {
  const hmac = self.hasher.hmac;
  const signKey = hmac(
    hmac(
      hmac(
        hmac("AWS4" + self.config.secretAccessKey, amzDate(ws.signDate, true), {
          hexOutput: false
        }),
        self.config.region,
...
function SimpleUriParser() {
  const parser = document ? document.createElement("a") : {};
...
function CryptoJSHasher() {
  return {
    ...
    hmac: function(key, input, options) {
      options = extend({ hexOutput: true, textInput: true }, options);
      const hmac = HmacSHA256(input, key, { asBytes: true });
      if (options.hexOutput) {
